...
async function findExistingIdByPublicLink(db, publicLink) {
  if (!publicLink) return null;
  const result = await db.execute("SELECT id, payload FROM collaborators");
  for (const row of result.rows) {
    try {
      const parsed = JSON.parse(String(row.payload || "{}"));
      if (parsed.publicLink && parsed.publicLink.toLowerCase() === publicLink.toLowerCase()) {
...
async function upsertCollaborator(collaboratorInput) {
  ...
  // If no explicit id was provided, check for an existing entry with the same publicLink
  // to avoid creating duplicates.
  const hasExplicitId = toText(collaboratorInput?.id).length > 0;
  if (!hasExplicitId) {
    const publicLink = toText(collaboratorInput?.publicLink || collaboratorInput?.mainLink || collaboratorInput?.link);
    const existingId = await findExistingIdByPublicLink(db, publicLink);
    if (existingId) {
